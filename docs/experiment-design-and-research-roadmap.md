...
## 8. 2026-08-14 设计更新：从时间单位到有界速度感知动力学

### 8.1 已证伪的两条路径

1. **原缩放下直接叠加多尺度损失**：三 seed 64/16 效应 gate 无可辨收益；事后解析审计确认该协议先天把初始逐帧位移压到约 1%，因此 no-go 只适用于该数值参数化。
2. **单位一致但只依赖位置的最小径向场**：恢复步幅后，5 epoch T3 只改善 `0.6575%`，且超过半数更新被裁剪；它既无法有效学习制动/转向，也不应继续靠学习率或 clip 扫描补救。

### 8.2 当前创新假设

状态必须显式包含速度，且速度通道同时满足对称性、耗散可解释性与数值有界性：

`a_i = a_conservative(x,z) + a_velocity(x,v,z) − γ_i v_i`

- `a_conservative` 与 `a_velocity` 均由 E(3) 不变量标量乘 E(3) 向量基构成；
- `γ_i = γ_max sigmoid(raw_i)`，从结构上限制过度阻尼；
- `|v|²` 采用 `log1p` 尺度，`v·r` 采用 `tanh` 饱和，冻结尺度来自训练集或预注册解析常数；
- 必须报告 `a_damp·v≤0`、保守/耗散加速度范数、step amplitude、RMSE、Phys 和裁剪率。

无界速度感知原型已经把 T3 RMSE 相对位置-only 降低 `80.5%`，但步幅塌到 `0.1135`，因此只支持“速度通道有效、阻尼约束必要”，不支持“当前模型可提交”。

### 8.3 下一阶段 gate

完整 MISATO 完成精确字节数、官方 MD5、HDF5、有限坐标和新版 split 验收后，按蛋白同源与配体 scaffold 分组建立新的 train/development 划分，再比较：

- A：position-only frame-time；
