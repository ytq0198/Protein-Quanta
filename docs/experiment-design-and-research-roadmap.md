...
3. 从头训练在 epoch 48 出现约 4531 的损失尖峰；之后稳定性从约 67% 降到约 52%，说明优化稳定性和轨迹几何稳定性是两个相关但不同的问题。
4. E3 的全局梯度裁剪 1.0 虽然将 408 个 batch 的更新限制在阈值内且没有非有限更新，最终验证 Stability 仍比未裁剪低 6.75 个百分点；所以“爆炸梯度”是症状之一，单独裁剪不是长程稳定性的充分解。
...
**阶段结果（2026-08-12）：E3 no-go。** 裁剪版 final 的验证 Stability/坐标 RMSE 为 45.52%/2.9152 Å，未裁剪 final 为 52.27%/2.6924 Å；两个成功条件均失败。最高裁剪前梯度范数达到 `4.04×10^8`，说明诊断有效，但固定阈值无法解决目标高方差和长程几何漂移。不再扫描 0.5/5.0，也不直接进入三 seed；保留裁剪和日志作为后续实验的安全设施，不把它作为创新贡献。
...
| E1 | 原始训练 seed 42 | 已完成 100 epoch | val 选 best | 暴露失败模式 |
| E2 | 锚点残差 β 扫描 | 无训练 | val 选 β，test 一次 | 出现 Pareto 改善 |
| E3 | 梯度裁剪 1.0 | 已完成 1×100；no-go | val | 未恢复稳定性，停止独立扩展 |
