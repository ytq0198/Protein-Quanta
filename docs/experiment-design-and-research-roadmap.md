...
| E3 | 梯度裁剪 1.0 | 1×100 epoch | val | 无爆炸且最终稳定性恢复 |
| E4 | 裁剪阈值 0.5/5.0（必要时） | 最多 2×100 | val | 选择单一阈值 |
| E5 | 三随机种子稳定训练 | 3×100 | val 汇总 | 均值/方差可信 |
...
若截止时间不足，优先级为 E2 → E3 → E5 → E6；E7/E8 保留初期结果和复赛计划，但不虚构完成度。
