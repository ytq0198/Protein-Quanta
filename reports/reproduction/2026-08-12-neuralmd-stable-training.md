...
本实验是严格单变量消融：相对 seed 42 的原始 100 epoch 复现，只增加全局 L2 梯度裁剪 `1.0`、裁剪前梯度范数日志和非有限更新保护。实验完整运行，但验证集结果未达到预注册条件，判定 **no-go**。因此未使用统一评估器新增测试集结果，也不继续扫描裁剪阈值。
...
4. 停止 E4 阈值扫描和 E5 三 seed；下一训练实验应改变信息更强的因素：`L_pair` 几何目标与受控跨度采样。只有单 seed 验证通过后才进行多 seed。
