...
> **2026-08-14 事后数值审计更正：**本结论只适用于本实验明确采用的“ODE 时间除以 100，但相邻帧差不乘 100”的 NeuralMD 上游多轨迹约定，不能外推为“多时间尺度监督对单位一致的三维动力学无效”。在零加速度解析情形下，该约定会使第一步位移只等于观测帧差的 `1/100`；这与本实验约 `0.009` 的 step-amplitude ratio 同量级。原始代码、检查点、数值与 no-go 判定均保留，但“模型表达能力不足”不再是唯一解释。后续必须先在 development-only 审计中比较原约定与 `initial_velocity_scale=100` 的单位一致版本，再决定是否启动新实验；不得据此重调已解封的 64/16 holdout。
...
这不是“创新损失未接入”的假阴性：三个 seed 的候选—控制参数 L2 分别为 `0.04874/0.06489/0.04416`。直接现象是最小等变场形成了近静态解：T1/T2/T3 的预测步幅仅为真值的约 `0.87%/0.98%/0.97%`。事后单位审计表明，`scaling=100` 与未缩放相邻帧差的组合本身就会把零加速度轨迹的逐帧位移压到 `1%`；因此当前证据只能说明多尺度监督没有克服该数值条件，不能单独归因于模型容量。停止原协议实验族，不据留出结果调系数或阈值。
